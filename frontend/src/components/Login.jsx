...
const Login = ({ onLogin, onRegisterClick, onForgotClick, onBack }) => {
    ...
    // NUEVO: Estado para saber si el servidor se está despertando
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        setLoading(true);
        setIsWakingUp(false); // Reiniciamos el estado

        // NUEVO: Si la petición tarda más de 3 segundos, asumimos que Render está "despertando"
        ...
        try {
            ...
            // Limpiamos el temporizador para que no se ejecute si la respuesta fue rápida
        ...
        >
            ...
            <div className="relative z-10 bg-white/10 backdrop-blur-md p-6 sm:p-10 rounded-2xl shadow-2xl border border-white/20 w-full max-w-md animacion-tarjeta">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-wide">Áurea Airlines</h1>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {/* Contenedor del botón y el mensaje de espera */}
                    <div className="pt-2">
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#E5B869] text-[#2A3F45] font-bold py-3 px-4 rounded-lg hover:bg-[#d4a556] transition-colors shadow-lg disabled:opacity-70 flex justify-center items-center gap-2"
                        >
                            {loading ? (
                                <>
                                    ...
                                    <span>{isWakingUp ? 'Conectando...' : 'Iniciando Sesión'}</span>
                                </>
                            ) : 'Iniciar Sesión'}
                        </button>

                        {/* NUEVO: Mensaje animado que aparece solo si tarda más de 3 segundos */}
                        {isWakingUp && (
                            <p className="text-[#E5B869] text-xs text-center mt-3 animate-pulse font-medium">
                                El servidor de Render está despertando. <br/> Puede tardar hasta 50 segundos ☕
                            </p>
                        )}
                    </div>
                </form>
            ...
            <div className="relative z-10 bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-white/20 w-full max-w-sm animacion-tarjeta" style={{ animationDelay: '0.1s' }}>
                ...
                <div className="pt-4 border-t border-white/20">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        {/* 4. Boton Backend (Apunta a Render endpoints) */}
                        <a
                            href="https://flight-booking-platform-x0rz.onrender.com/api/flights"
                            target="_blank" rel="noopener noreferrer"
