    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(`${API_URL}/users/register`, { 
                name, 
                email, 
                phone,  // Se envía el teléfono al backend
                password 
            });
            onRegisterSuccess();
        } catch (err) {
        ...
        <AuthLayout onBack={onBackToWelcome}>
            <div className="bg-white/10 backdrop-blur-md p-6 sm:p-10 rounded-2xl shadow-2xl border border-white/20 w-full max-w-md my-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-white/20 border border-white/10 rounded-lg p-2.5 sm:p-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#E5B869] transition-all text-sm" 
                            required 
                        />
                    </div>
                ...
                <div className="text-center mt-6 pt-4 border-t border-white/20">
                    <button 
                        onClick={onGoToLogin} 
                        className="text-sm text-gray-300 hover:text-white transition-colors flex items-center justify-center w-full gap-2"
                    >
